...
User.findOneUser = async (req, res) => {
    try {
        const { body } = req;
        let table = await Users.findOne({ where: { id: body.userId}  , include:[
            {
                model:Videos,
                include:[
                    {
...
User.updateTable = async (req, res) => {
    try {
        ...
        let table = await Users.update({ avatar: body.avatar , name:body.name , email:body.email }, { where: { id: params.id } });
